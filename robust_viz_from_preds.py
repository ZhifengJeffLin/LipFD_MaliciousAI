...
# Read ./work/robust_eval/*/preds_per_sample.csv and combine them into compact summary plots.
...
def infer_label(sample_id: str) -> int:
    """Infer ground-truth label from sample_id."""
    s = str(sample_id)
...
def load_metrics(csv_path: str):
    """Load per-sample prediction CSV and compute AUC, EER, thresholds, and confusion metrics."""
    ...
    idx = int(np.nanargmin(np.abs(fpr - (1 - tpr))))  # EER point
...
def main():
    ...
    ap.add_argument("--base", default="./work/robust_eval",
                    help="Root directory containing subfolders clean/q80/... etc.")
    ap.add_argument("--splits", nargs="*", default=None,
                    help="Names of subfolders to process; by default auto-discovered and sorted by clean,q80,q60,q40")
    args = ap.parse_args()
    ...
    # Figure 2: AUC/EER combined line plot
    ...
    # Figure 3: Score distributions grid (one subplot per split)
    n = len(names)
    rows = math.ceil(n / 2)
    ...
    # Figure 4: Confusion matrix grid (unified color scale + horizontal colorbar)
    vmax = 0
    cms = {}
    for name, res in results.items():
    ...
    for i, name in enumerate(names):
        ax = axes[i]
        cm = cms[name]
        im = ax.imshow(cm, vmin=0, vmax=vmax, cmap="viridis")  # unified color scale
        for r in range(2):
            for c in range(2):
    ...
    # Add horizontal colorbar at the bottom (leaving enough space)
    fig.subplots_adjust(left=0.08, right=0.98, top=0.92, bottom=0.16, wspace=0.25, hspace=0.35)
    cbar_ax = fig.add_axes([0.18, 0.08, 0.64, 0.04])  # [left, bottom, width, height] in figure coordinates
